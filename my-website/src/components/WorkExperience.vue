<template>
    <div class="max-w-2xl mx-auto p-6 my-36 text-amber-950">
        <h1 class="text-4xl font-bold text-center mb-8">Work Experience</h1>

        <div class="relative border-l-2 border-gray-400 ml-3">
            <div class="mb-8 ml-6">
                <span
                    class="absolute flex items-center justify-center w-2 h-2 bg-stone-500 rounded-full -left-1 ring-2 ring-white"
                >
                </span>
                <h3 class="text-lg font-semibold">
                    基隆市警察局第二分局八斗子分駐所
                </h3>
                <time class="block mb-2 text-sm text-gray-500"
                    >2019年10月 - 2025年1月</time
                >
                <p class="text-gray-700">擔任警員，從事警察六大任務。</p>
                <p class="text-gray-700">
                    辦理竊盜、詐欺、恐嚇、公共危險、毒品、等刑案
                </p>
                <p class="text-gray-700">
                    辦理刑事相關業務、犯罪預防宣導、防詐意識宣導..等
                </p>
                <p class="text-gray-700">
                    辦理婦幼相關業務、小小警察活動、性騷擾防治宣導、性影像剝削宣導、性侵害加害人管制...等
                </p>
            </div>

            <div class="mb-8 ml-6">
                <span
                    class="absolute flex items-center justify-center w-2 h-2 bg-stone-500 rounded-full -left-1 ring-2 ring-white"
                >
                </span>
                <h3 class="text-lg font-semibold">
                    加入文大推廣 x 程式驅動 全端設計工程師養成班
                </h3>
                <time class="block mb-2 text-sm text-gray-500"
                    >2025年4月 - 2025年8月</time
                >
                <p class="text-gray-700">
                    學習前端、後端、資料庫、雲端平台、容器化等技術，參與專案開發
                </p>
            </div>
        </div>
    </div>
</template>
<script setup></script>
<style scoped></style>
